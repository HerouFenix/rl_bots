...
class Primus(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket):
        ...
        if self.play is None:
            ...
            #self.play = strategy.choose_play(self.info, self.info.cars[self.index])
            self.play = Drive(self.info.cars[self.index], target_speed=5000)

        # If bot has picked a play, execute it
        if self.play is not None:
            self.play.step(self.info.time_delta)
